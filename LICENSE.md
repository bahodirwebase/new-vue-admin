# License Agreement

## Byxora - Vue 3 Admin Dashboard

This license grants you the right to use this product in accordance with the terms and conditions outlined below.

---

## 📄 License Types

### Regular License
- **Use**: For one end product (one domain)
- **Commercial Use**: Allowed
- **Resale**: Not allowed
- **Support**: 6 months included
- **Updates**: 6 months included

### Extended License  
- **Use**: For one end product (one domain)
- **Commercial Use**: Allowed
- **Resale**: Allowed (as part of larger work)
- **Support**: 12 months included
- **Updates**: 12 months included

---

## 🎯 What You Can Do

✅ **Use in personal projects**
✅ **Use in commercial projects**
✅ **Modify the code**
✅ **Create custom themes**
✅ **Add new features**
✅ **Use in SaaS applications**
✅ **Use in client projects**
✅ **Distribute as part of your product** (Extended License)

---

## 🚫 What You Cannot Do

❌ **Resell as standalone product** (Regular License)
❌ **Redistribute on theme marketplaces**
❌ **Claim ownership of the original code**
❌ **Remove copyright notices**
❌ **Use in illegal or harmful applications**
❌ **Distribute under different license terms**

---

## 📜 Terms and Conditions

### 1. License Grant
This license grants you a non-exclusive, non-transferable right to use the Byxora Admin Dashboard in accordance with these terms.

### 2. Usage Rights
You may use this product to create unlimited websites and applications for yourself or your clients.

### 3. Modification Rights
You are permitted to modify the source code to suit your needs. However, you must maintain the original copyright notices.

### 4. Redistribution
Under the Regular License, you cannot redistribute this product as a standalone item. Under the Extended License, you may include it in larger works.

### 5. Support and Updates
Support and updates are included for the duration specified in your license type. After this period, you may renew to continue receiving support.

### 6. Warranty
This product is provided "as is" without warranty of any kind. The author is not liable for any damages arising from its use.

### 7. Termination
This license is effective until terminated. Your rights under this license will terminate automatically if you fail to comply with any terms.

### 8. Governing Law
This license is governed by the laws of the jurisdiction in which the product was purchased.

---

## 🔄 Renewal and Upgrades

### License Renewal
- Extend support and updates
- Access to new features
- Continued technical support
- Discounted renewal rates

### Upgrade to Extended
- Upgrade from Regular to Extended license
- Pay only the difference
- Get extended usage rights

---

## 📞 Support

For license-related questions:
- 📧 Email: license@example.com
- 📖 Documentation: [Link to docs]
- 💬 Live Chat: Available on website

---

## 🎁 Bonus Features

With your license purchase, you get:
- 📚 Comprehensive documentation
- 🎨 Multiple theme variations
- 📱 Responsive design
- 🔧 Regular updates
- 💬 Technical support
- 📈 Performance optimizations
- 🛡️ Security updates

---

## 📄 Legal Information

**Product Name**: Byxora - Vue 3 Admin Dashboard  
**Version**: 1.0.0  
**Release Date**: January 15, 2024  
**Copyright**: © 2024 [Your Company Name]  
**License Type**: Regular/Extended  

---

## 🤝 Fair Use Policy

We believe in fair use and want our customers to succeed. This license is designed to:
- ✅ Protect our intellectual property
- ✅ Enable your success
- ✅ Ensure sustainable development
- ✅ Provide ongoing value

---

## 📝 Questions?

If you have any questions about this license:
1. Check our FAQ section
2. Review the documentation
3. Contact our support team
4. Consult with legal professional if needed

---

**Thank you for choosing Byxora Admin Dashboard!**

*This license is a legal agreement between you and [Your Company Name]. By using this product, you agree to be bound by these terms and conditions.*
